...
// matrix-js-sdk's Rust crypto store is a SINGLE IndexedDB per origin (fixed
// "matrix-js-sdk" prefix; client.initRustCrypto() doesn't let us change it, and
// bypassing it to set a per-subnet prefix duplicates the crypto WASM module and
// breaks sync). So it holds one matrix account at a time. We record which userId
// owns it; when a different account logs in (switching subnets) we wipe the
// crypto IndexedDB first so initRustCrypto starts clean instead of throwing an
// account mismatch. Trade-off: the previous subnet's E2E keys are dropped on
// each switch (no key backup configured to restore them).
const RUST_CRYPTO_DB_PREFIX = 'matrix-js-sdk';
const RUST_CRYPTO_OWNER_KEY = 'subnet-client::rust-crypto-owner';

// Best-effort delete of the matrix-js-sdk Rust crypto IndexedDB databases;
// resolves even if a delete is blocked by an open connection.
async function delete_rust_crypto_dbs() {
  if (typeof indexedDB === 'undefined' || !indexedDB.databases) return;
  const dbs = await indexedDB.databases().catch(() => []);
  await Promise.all(dbs.map((db) => new Promise((resolve) => {
    if (!db.name || !db.name.startsWith(RUST_CRYPTO_DB_PREFIX)) { resolve(); return; }
...
class E2EMatrixClient {
  ...
  async _getSecretStorageKey() {
    ...
    this._ssKey = await this._deriveSecretStorageKey();
    this.kv.saveBackupKey(this._ssKey);
    return this._ssKey;
  }
  ...
  async login(username, password, opts = {}) {
    ...
    if (!usedStoredToken) {
      ...
      if (deviceId && newDeviceId !== deviceId) {
        // The device rotated (e.g. the stored token expired and the server issued
        // a new device). The existing crypto store is bound to the old device, so
        // wipe it and continue — room keys are restored from key backup below.
        await delete_rust_crypto_dbs();
      }
    ...
    // The Rust crypto store is shared across subnets (one per origin). If it
    // currently holds a different account, wipe it before opening so initRustCrypto
    // doesn't throw "the account in the store doesn't match the account in the
    // constructor" when switching subnets. We use the client's own (public)
    // initRustCrypto so there's a single crypto/WASM instance — bypassing it to set
    // a per-subnet prefix duplicates the WASM module and hangs sync.
    const cryptoOwner = globalThis.localStorage?.getItem(RUST_CRYPTO_OWNER_KEY);
    if (cryptoOwner && cryptoOwner !== userId) {
      await delete_rust_crypto_dbs();
    }
    globalThis.localStorage?.setItem(RUST_CRYPTO_OWNER_KEY, userId);

    // Initialize the Rust crypto module (loads or creates the IndexedDB crypto
    // store keyed by user_id+device_id) before startClient so the first /sync's
    // to-device events are decrypted in place.
    await this.client.initRustCrypto();

    // Cross-signing + secret storage + key backup. Cross-signing marks all your
    // devices as "you"; secret storage (SSSS) holds the cross-signing private keys
    // and the key-backup decryption key, encrypted with a key derived from your
    // wallet; key backup stores Megolm room keys (encrypted) on the homeserver so
    // history decrypts on any browser. EIP-191 sign-in already proved wallet
    // ownership, so we may set these up. Best-effort: sign-in must still succeed if
    // any step fails (e.g. a homeserver without key backup).
    try {
      const crypto = this.client.getCrypto();
      if (crypto) {
        // 1. Cross-signing. With getSecretStorageKey wired, bootstrap imports the
        //    existing keys from SSSS on a new device; otherwise it creates them.
        //    Authenticate the device-signing-key upload with the matrix password.
        ...
        // 2. Secret storage + key backup. Only create when absent, so we never
        //    reset another device's SSSS. createSecretStorageKey returns our
        //    wallet-derived key so every device/browser derives the same one.
        if (!(await crypto.isSecretStorageReady().catch(() => false))) {
        ...
        // 3. Load the backup decryption key from SSSS, enable the backup engine,
        //    and restore room keys in the background so history decrypts.
        await crypto.loadSessionBackupPrivateKeyFromSecretStorage().catch(() => {});
        const backup = await crypto.checkKeyBackupAndEnable().catch(() => null);
        if (backup) {
          crypto.restoreKeyBackup().catch((e) =>
            console.warn('[E2E] key backup restore failed:', e?.message));
        }
        this._crossSigningStatus = { state: 'ready' };
      }
    } catch (e) {
      console.warn('[E2E] crypto bootstrap skipped:', e.message);
    }
