...
class E2EMatrixClient {
  ...
  async login(username, password, opts = {}) {
    ...
    if (!usedStoredToken) {
      ...
      if (deviceId && cryptoMatchesSession && newDeviceId !== deviceId) {
        // Rare: device rotated despite a matching crypto store (the device was
        // deleted server-side). Wipe THIS subnet's store; key backup restores keys.
