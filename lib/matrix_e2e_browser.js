...
class E2EMatrixClient {
  ...
  // your devices as "you"; secret storage (SSSS) holds the cross-signing + backup
  // keys encrypted with the wallet-derived key; key backup stores Megolm room keys
  // (encrypted) on the homeserver so history decrypts on any browser. Best-effort.
  async _bootstrapCryptoExtras(userId, password) {
    const crypto = this.client.getCrypto();
    if (!crypto) return;
    // 1. Cross-signing. Imports existing keys from SSSS on a new device if
    //    available; otherwise creates them, authenticating the upload with the
    //    matrix password.
    ...
    // 2. Secret storage + key backup. Only create when absent, so we never reset
    //    another device's SSSS. createSecretStorageKey returns our wallet-derived
    //    key so every device/browser derives the same one.
    ...
    // 3. Load the backup decryption key from SSSS, enable the backup engine, and
    //    restore room keys so history decrypts.
