...
// give each one its own localStorage session namespace AND its own Rust crypto
// store, so switching subnets never loads the wrong session or collides on the
// shared crypto account.
function matrix_host(url) {
  try { return new URL(url).host; } catch { return url; }
}

// The matrix-js-sdk Rust crypto store is normally a single IndexedDB per origin
// (fixed "matrix-js-sdk" prefix, not configurable through client.initRustCrypto).
// We init it ourselves with a per-subnet prefix so each subnet keeps its own
// persistent E2E keys. These are the crypto events client.initRustCrypto re-emits
// onto the client; we replicate that so verification/key-backup UI still works.
const RE_EMITTED_CRYPTO_EVENTS = [
  'VerificationRequestReceived', 'UserTrustStatusChanged', 'KeyBackupStatus',
  'KeyBackupSessionsRemaining', 'KeyBackupFailed', 'KeyBackupDecryptionKeyCached',
  'KeysChanged', 'DevicesUpdated', 'WillUpdateDevices', 'DehydratedDeviceCreated',
  'DehydratedDeviceUploaded', 'RehydrationStarted', 'RehydrationProgress',
  'RehydrationCompleted', 'RehydrationError', 'DehydrationKeyCached',
...
class E2EMatrixClient {
  ...
  constructor({ matrixUrl, privateKey, signer, userAddress, storePath, delegation }) {
    ...
    // Per-subnet Rust crypto store prefix (see _initRustCrypto). Distinct IndexedDB
    // per subnet → E2E keys survive across switches instead of colliding.
    this.cryptoStorePrefix = `${this.storePath}::crypto`;
  ...
  // Initialize matrix-js-sdk's Rust crypto with a PER-SUBNET store prefix.
  // client.initRustCrypto() hardcodes a single origin-wide IndexedDB prefix, so
  // every subnet would share one crypto store and collide on account/device.
  // Instead we call the lower-level RustCrypto.initRustCrypto with our own
  // per-subnet prefix and replicate the exact attach wiring the client does
  // (crypto backend + sync event listeners + event re-emit), so each subnet has
  // its own persistent E2E keys and switching never wipes them.
  async _initRustCrypto(userId, deviceId) {
    const client = this.client;
    if (client.cryptoBackend) return;
    const sdk = await this._loadSdk();
    const { initRustCrypto } = await import('matrix-js-sdk/lib/rust-crypto/index.js');
    const rustCrypto = await initRustCrypto({
      logger: client.logger,
      http: client.http,
      userId,
      deviceId,
      secretStorage: client.secretStorage,
      cryptoCallbacks: client.cryptoCallbacks,
      storePrefix: this.cryptoStorePrefix,
      storeKey: undefined,
      storePassphrase: undefined,
      legacyCryptoStore: undefined,
      legacyPickleKey: 'DEFAULT_KEY',
    });
    rustCrypto.setSupportedVerificationMethods(client.verificationMethods);
    client.cryptoBackend = rustCrypto;
    client.on(sdk.RoomMemberEvent.Membership, rustCrypto.onRoomMembership.bind(rustCrypto));
    client.on(sdk.ClientEvent.Event, (event) => rustCrypto.onLiveEventFromSync(event));
    const reEmit = RE_EMITTED_CRYPTO_EVENTS.map((k) => sdk.CryptoEvent[k]).filter(Boolean);
    client.reEmitter.reEmit(rustCrypto, reEmit);
  }
  ...
  async login(username, password, opts = {}) {
    ...
    // to-device events are decrypted in place. We use our own per-subnet store
    // prefix (instead of client.initRustCrypto's fixed one) so each subnet keeps
    // a separate, persistent crypto store and switching never wipes E2E keys.
    await this._initRustCrypto(userId, deviceId);
