...
class E2EMatrixClient {
  ...
  async _projectEvent(event, encrypted, displayNames, roomId) {
    ...
    // decryption then fails and it would surface as "[unable to decrypt]" instead
    // of vanishing. Drop it outright.
    if (event.unsigned?.redacted_because) return null;
    ...
    if (FILE_TYPES.includes(msgtype)) {
      ...
      const fileRel = msgEvent.content['m.relates_to'];
      if (fileRel) {
        if (fileRel.rel_type === 'm.thread') {
          fileResult.thread_id = fileRel.event_id;
          if (fileRel['m.in_reply_to']?.event_id) fileResult.reply_to = fileRel['m.in_reply_to'].event_id;
        } else if (fileRel['m.in_reply_to']?.event_id) {
          fileResult.reply_to = fileRel['m.in_reply_to'].event_id;
        }
      }
      return fileResult;
    }
  ...
  async sendMessage(roomId, text, opts = {}) {
    ...
    if (opts.threadRootId) {
      // Threads are flat: if the target is already in a thread, walk up to that
      // thread's canonical root so we never create a nested thread — the message
      // lands in the existing thread. in_reply_to keeps the message replied to.
      // m.relates_to is cleartext even in encrypted rooms, so the raw fetch sees it.
      let rootId = opts.threadRootId;
      const seen = new Set();
      for (let i = 0; i < 20 && rootId && !seen.has(rootId); i++) {
        seen.add(rootId);
        let ev = null;
        try { ev = await this.client.fetchRoomEvent(roomId, rootId); } catch { break; }
        const rel = ev && ev.content ? ev.content['m.relates_to'] : null;
