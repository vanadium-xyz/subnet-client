...
- Subnet API actions that require a fresh main-key signature — e.g. `update-metadata` — **will not work** on the bootstrapped device until you attach the real wallet. The CLI / agora will return "Main signer not available on this device" for those calls.
